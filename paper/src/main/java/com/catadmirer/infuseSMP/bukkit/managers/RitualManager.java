...
public class RitualManager {
    ...
    public RitualManager() {
        plugin = InfusePlugin.getInstance();
    }
    ...
    public void startRitual(HumanEntity player, InfuseEffect effect, Location location) {
        ...
        bossBar = BossBar.bossBar(bossBarName, 1, effect.ritualColor(), BossBar.Overlay.PROGRESS);
    ...
    public void completeRitual() {
        ...
        // Dropping the item
        location.getWorld().dropItem(location.add(0, 1, 0), ((BukkitEffect) effect).createItem());

        // Updating the ender effect recipe
        if (effect.id() == EffectConstants.Id.ENDER.value()) plugin.getRecipeManager().updateEnderRecipe();
